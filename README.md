...
Пароли хранятся в `appsettings.Development.json` (этот файл не попадает в git). 
В `appsettings.json` массив пустой — это безопасно коммитить.
...
💡 Доступные роли: User (по умолчанию) или Admin.
Для полного доступа (создание/удаление событий) используйте роль Admin.

3. Нажмите Execute — ответ **204 No Content** означает успешную регистрацию.
...
> **HTTP 204 No Content** — пользователь зарегистрирован  
