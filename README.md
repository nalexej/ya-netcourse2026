...
В стек включены четыре инструмента для сбора метрик, трассировки, логирования и визуализации:
...
| **Serilog** | Структурированное логирование (JSON) | — | Консоль / файлы |
...
**Serilog** пишет структурированные JSON-логи в консоль и файлы. Каждый лог содержит стандартные поля (`Timestamp`, `Level`, `MessageTemplate`, `Properties`) и кастомные поля (ID события, ID бронирования, имя сервиса), что позволяет удобно искать и фильтровать записи в консоли или отправлять в Grafana Loki.
